...
static void _dwt_otpprogword32(
    uint32_t data, uint16_t address);  // Program the non-volatile memory
...
typedef struct {
  ...
  uint16_t
      sleep_mode;  // Used for automatic reloading of LDO tune and microcode at wake-up
  uint16_t
      ststhreshold;  // Threshold for deciding if received STS is good or bad
...
static void dwt_xfer3000(
  ...
  header[1] = (uint8_t)(
      addr | (mode & 0x03));  // & 0xFF; //EAM: subaddr[5:0]+ R/W/AND_OR
  ...
  if (/*reg_offset == 0 && */ length == 0) { /* Fast Access Commands (FAC)
    ...
    cnt = 1;
      ...
      reg_offset == 0 /*&& length > 0*/ &&
      (mode == DW3000_SPI_WR_BIT ||
       mode ==
           DW3000_SPI_RD_BIT)) { /* Fast Access Commands with Read/Write support (FACRW)
...
*/
uint8_t dwt_generatecrc8(const uint8_t *byteArray, uint8_t len,
...
void dwt_enablespicrccheck(dwt_spi_crc_mode_e crc_mode,
  ...
  if (crc_mode != DWT_SPI_CRC_MODE_NO)  //enable CRC check in DW3000
  {
    ...
    if (crc_mode ==
        DWT_SPI_CRC_MODE_WRRD)  //enable CRC generation on SPI read transaction which the DW3000 will store in SPICRC_CFG_ID register
    {
...
int dwt_initialise(uint8_t mode) {
  ...
  pdw3000local->sleep_mode =
      DWT_RUNSAR;  // Configure RUN_SAR on wake by default as it is needed when running PGF_CAL
  ...
  if (pdw3000local->tempP ==
      0)  //if the reference temperature has not been programmed in OTP (early eng samples) set to default value
  {
    pdw3000local->tempP = 0x85;  //@temp of 20 deg
  }

  if (pdw3000local->vBatP ==
      0)  //if the reference voltage has not been programmed in OTP (early eng samples) set to default value
  {
...
void dwt_setdwstate(uint8_t state) {
  if (state ==
      DWT_DW_IDLE)  // Set the auto INIT2IDLE bit so that DW3000 enters IDLE mode before switching clocks to system_PLL
  //NOTE: PLL should be configured prior to this, and the device should be in IDLE_RC (if the PLL does not lock device will remain in IDLE_RC)
  {
    ...
    dwt_or8bitoffsetreg(SEQ_CTRL_ID, 0x01, SEQ_CTRL_AINIT2IDLE_BIT_MASK >> 8);
      ...
      state ==
      DWT_DW_IDLE_RC)  //Change state to IDLE_RC and clear auto INIT2IDLE bit
  {
...
int dwt_configure(dwt_config_t *config) {
  ...
  if (config->txPreambLength == DWT_PLEN_72) {
    dwt_setplenfine(
        8);  //value 8 sets fine preamble length to 72 symbols - this is needed to set 72 length.
  } else {
  ...
  dwt_write8bitoffsetreg(PLL_CAL_ID, 0,
                         RF_PLL_CFG_LD);  // Extend the lock delay
  ...
  // PGF
  error = dwt_pgf_cal(
      1);  //if the RX calibration routine fails the device receiver performance will be severely affected, the application should reset and try again
...
int dwt_pgf_cal(uint8_t ldoen) {
  ...
  if (ldoen == 1) {
    ...
    dwt_or16bitoffsetreg(
        LDO_CTRL_ID, 0,
        (LDO_CTRL_LDO_VDDIF2_EN_BIT_MASK | LDO_CTRL_LDO_VDDMS3_EN_BIT_MASK |
  ...
  if (txBufferOffset <= 127) {
    ...
    dwt_modify32bitoffsetreg(TX_FCTRL_ID, 0,
                             ~(TX_FCTRL_TXB_OFFSET_BIT_MASK |
                               TX_FCTRL_TR_BIT_MASK | TX_FCTRL_TXFLEN_BIT_MASK),
                             reg32);
    reg32 = dwt_read8bitoffsetreg(
        SAR_CTRL_ID,
        0);  //DW3000/3700 - need to read this to load the correct TX buffer offset value
  }
...
void dwt_readrxdata(uint8_t *buffer, uint16_t length, uint16_t rxBufferOffset) {
  ...
  if (pdw3000local->dblbuffon ==
      DBL_BUFF_ACCESS_BUFFER_1)  //if the flag is 0x3 we are reading from RX_BUFFER_1
  {
...
int16_t dwt_readclockoffset(void) {
  ...
  switch (
      pdw3000local
          ->dblbuffon)  //if the flag is non zero - we are either accessing RX_BUFFER_0 or RX_BUFFER_1
  {
    ...
    case DBL_BUFF_ACCESS_BUFFER_0:
      ...
      break;
    default:
      regval = dwt_read16bitoffsetreg(CIA_DIAG_0_ID, 0) &
               CIA_DIAG_0_COE_PPM_BIT_MASK;
      break;
  }
...
int dwt_readstsquality(int16_t *rxStsQualityIndex) {
  ...
  preambleCount =
      ...
      STS_STS_ACC_QUAL_BIT_MASK;  //  dwt_read16bitoffsetreg(CP_PRNG_ID, CP_STS_OFFSET) & CP_ACC_CP_QUAL_MASK;
...
int dwt_readstsstatus(uint16_t *stsStatus, uint8_t sts_num) {
  ...
  switch (
      pdw3000local
          ->dblbuffon)  //check if in double buffer mode and if so which buffer host is currently accessing
  {
    case DBL_BUFF_ACCESS_BUFFER_1:
      //!!! Assumes that Indirect pointer register B was already set. This is done in the dwt_setdblrxbuffmode when mode is enabled.
      *stsStatus =
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics) {
  ...
  uint8_t temp
      [DB_MAX_DIAG_SIZE];  //address from 0xC0000 to 0xD0068 (108*2 bytes) - when using normal mode, or 232 length for max logging when in Double Buffer mode
          ...
          ->dblbuffon)  //check if in double buffer mode and if so which buffer host is currently accessing
  {
    ...
    case DBL_BUFF_ACCESS_BUFFER_0:
      ...
      for (i = 0; i < (CIA_I_RX_TIME_LEN + 1); i++) {
        diagnostics->tdoa[i] = temp
            [i + BUF0_TDOA -
             BUF0_RX_FINFO];  // timestamp difference of the 2 STS RX timestamps
      }
      ...
      diagnostics->ipatovPOA =
          (uint16_t)temp[BUF0_IP_TS - BUF0_RX_FINFO + 2] << 8 |
          temp[BUF0_IP_TS - BUF0_RX_FINFO +
               1];  // Phase of arrival as computed from the Ipatov CIR (signed rad*2-12)

      diagnostics->stsRxStatus =
          temp[BUF0_STS_TS - BUF0_RX_FINFO +
               CIA_C_STAT_OFFSET];  // RX status info for STS
      diagnostics->stsPOA =
          (uint16_t)temp[BUF0_STS_TS - BUF0_RX_FINFO + 2] << 8 |
          temp[BUF0_STS_TS - BUF0_RX_FINFO +
               1];  // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)

      diagnostics->sts2RxStatus =
          temp[BUF0_STS1_TS - BUF0_RX_FINFO +
               CIA_C_STAT_OFFSET];  // RX status info for STS1
      diagnostics->sts2POA =
          (uint16_t)temp[BUF0_STS1_TS - BUF0_RX_FINFO + 2] << 8 |
          temp[BUF0_STS1_TS - BUF0_RX_FINFO +
               1];  // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)
      ...
      diagnostics->stsPower =
          ((uint16_t)temp[BUF0_STS_DIAG_1 - BUF0_RX_FINFO + 1] << 8 |
           temp[BUF0_STS_DIAG_1 -
                BUF0_RX_FINFO]);  // channel area allows estimation of channel power for the STS
      diagnostics->stsF1 =
    ...
    default:  //double buffer is off
      ...
      diagnostics->ipatovPOA =
          (uint16_t)temp[IP_TOA_HI_ID - IP_TOA_LO_ID + 2] << 8 |
          temp[IP_TOA_HI_ID - IP_TOA_LO_ID +
               1];  // Phase of arrival as computed from the Ipatov CIR (signed rad*2-12)

      diagnostics->stsRxStatus =
          ((uint16_t)
               temp[STS_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET + 1] |
           temp[STS_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET]) >>
          7;  // RX status info for STS
      diagnostics->stsPOA =
          (uint16_t)temp[STS_TOA_HI_ID - IP_TOA_LO_ID + 2] << 8 |
          temp[STS_TOA_HI_ID - IP_TOA_LO_ID +
               1];  // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)

      diagnostics->sts2RxStatus =
          ((uint16_t)
               temp[STS1_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET + 1] |
           temp[STS_TOA_HI_ID - IP_TOA_LO_ID + CIA_C_STAT_OFFSET]) >>
          7;  // RX status info for STS
      diagnostics->sts2POA =
          (uint16_t)temp[STS1_TOA_HI_ID - IP_TOA_LO_ID + 2] << 8 |
      ...
      diagnostics->stsPower =
          ((uint16_t)temp[STS_DIAG_1_ID - IP_TOA_LO_ID + 1] << 8 |
           temp[STS_DIAG_1_ID -
                IP_TOA_LO_ID]);  // channel area allows estimation of channel power for the STS
      diagnostics->stsF1 =
      ...
      diagnostics->stsF3 =
          ((uint32_t)temp[STS_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 3]
               << 24 |
           (uint32_t)temp[STS_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 2]
               << 16 |
      ...
      diagnostics->stsFpIndex =
          ...
          0x7FFF;  // First path index [14:0] for STS
      diagnostics->stsAccumCount =
          ((uint16_t)temp[STS_DIAG_12_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
      ...
      //STS 2
      diagnostics->sts2Peak =
          ((uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd + 3]
               << 24 |
           (uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd + 2]
               << 16 |
           (uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
           (uint32_t)temp[STS1_DIAG_0_ID - STS_DIAG_4_ID + offset_0xd]) &
          0x3FFFFFFF;  // index [29:21] and amplitude [20:0] of peak sample in STS CIR
      diagnostics->sts2Power =
          ((uint16_t)temp[STS1_DIAG_1_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
           temp[STS1_DIAG_1_ID - STS_DIAG_4_ID +
                offset_0xd]);  // channel area allows estimation of channel power for the STS
      diagnostics->sts2F1 =
          ((uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd + 3]
               << 24 |
           (uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd + 2]
               << 16 |
           (uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
           (uint32_t)temp[STS1_DIAG_2_ID - STS_DIAG_4_ID + offset_0xd]) &
          0x3FFFFF;  // F1 for STS [21:0]
      diagnostics->sts2F2 =
          ((uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd + 3]
               << 24 |
           (uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd + 2]
               << 16 |
           (uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
           (uint32_t)temp[STS1_DIAG_3_ID - STS_DIAG_4_ID + offset_0xd]) &
          0x3FFFFF;  // F2 for STS [21:0]
      diagnostics->sts2F3 =
          ((uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 3]
               << 24 |
           (uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 2]
               << 16 |
           (uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
           (uint32_t)temp[STS1_DIAG_4_ID - STS_DIAG_4_ID + offset_0xd]) &
          0x3FFFFF;  // F3 for STS [21:0]
      diagnostics->sts2FpIndex =
          ((uint16_t)temp[STS1_DIAG_8_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
           temp[STS1_DIAG_8_ID - STS_DIAG_4_ID + offset_0xd]) &
          0x7FFF;  // First path index [14:0] for STS
      diagnostics->sts2AccumCount =
          ((uint16_t)temp[STS1_DIAG_12_ID - STS_DIAG_4_ID + offset_0xd + 1]
               << 8 |
...
uint32_t dwt_readtxtimestamplo32(void) {
  return dwt_read32bitreg(
      TX_TIME_LO_ID);  // Read TX TIME as a 32-bit register to get the 4 lower bytes out of 5
}
          ...
          ->dblbuffon)  //check if in double buffer mode and if so which buffer host is currently accessing
  {
    ...
    default:
      pdoa = dwt_read16bitoffsetreg(CIA_TDOA_1_PDOA_ID, 2) &
             (CIA_TDOA_1_PDOA_PDOA_BIT_MASK >>
              16);  // phase difference of the 2 POAs
      break;
  }
...
void dwt_readtdoa(uint8_t *tdoa) {
  ...
  tdoa[5] &=
      0x01;  // TDOA value is 41 bits long. You will need to read 6 bytes and mask the highest byte with 0x01
}
...
uint32_t dwt_readrxtimestamplo32(void) {
  return dwt_read32bitreg(
      RX_TIME_0_ID);  // Read RX TIME as a 32-bit register to get the 4 lower bytes out of 5 byte timestamp
}
...
static void _dwt_otpprogword32(uint32_t data, uint16_t address) {
  ...
  dwt_write16bitoffsetreg(OTP_WDATA_ID, 0,
                          0x0002);  // Different to previous one
  ...
  deca_sleep(
      2);  //Uncomment this command if you don't want to use the loop above. It will take more time than the loop above.
...
uint16_t dwt_calibratesleepcnt(void) {
  ...
  deca_sleep(
      2);  //need to wait for at least 1 LP OSC period at slowest frequency of 15kHz =~ 66 us
...
void dwt_setdblrxbuffmode(dwt_dbl_buff_state_e dbl_buff_state,
  ...
  if (dbl_buff_state == DBL_BUF_STATE_EN) {
    ...
    pdw3000local->dblbuffon =
        DBL_BUFF_ACCESS_BUFFER_0;  //the host will access RX_BUFFER_0 initially (on 1st reception after enable)
...
void dwt_setrxaftertxdelay(uint32_t rxDelayTime) {
  ...
  val |=
      (rxDelayTime &
       ACK_RESP_W4R_TIM_BIT_MASK);  // In UWB microseconds (e.g. turn the receiver on 20uus after TX)
...
void dwt_isr(void) {
  ...
  if (fstat & FINT_STAT_RXOK_BIT_MASK) {
    ...
    if (pdw3000local->dblbuffon)  // if in double buffer mode
    {
      ...
      if (pdw3000local->dblbuffon ==
          DBL_BUFF_ACCESS_BUFFER_1)  //If accessing the second buffer (RX_BUFFER_B then read second nibble of the DB status reg)
      {
        statusDB >>= 4;
      }
    ...
    if ((status & SYS_STATUS_RXFR_BIT_MASK) &&
              ...
              ->dblbuffon)  //check if in double buffer mode and if so which buffer host is currently accessing
      {
        ...
        case DBL_BUFF_ACCESS_BUFFER_0:  //accessing frame info relating to the first buffer (RX_BUFFER_0)
          ...
          break;
        default
            :  //accessing frame info relating to the second buffer (RX_BUFFER_0) (single buffer mode)
    ...
    dwt_write32bitreg(
        SYS_STATUS_ID,
        cia_err |
            SYS_STATUS_ALL_RX_GOOD);  // Clear all status bits relating to good reception
  ...
  if (fstat & FINT_STAT_RXERR_BIT_MASK) {
    ...
    dwt_write32bitoffsetreg(
        SYS_STATUS_ID, 0, SYS_STATUS_ALL_RX_ERR);  // Clear RX error event bits
...
static void dwt_force_clocks(int clocks) {
  ...
  if (clocks == FORCE_CLK_AUTO) {
    ...
    dwt_write16bitoffsetreg(
        CLK_CTRL_ID, 0x0,
        (uint16_t)
            DWT_AUTO_CLKS);  //we only need to restore the low 16 bits as they are the only ones to change as a result of  FORCE_CLK_SYS_TX
  }
...
void dwt_setdelayedtrxtime(uint32_t starttime) {
  dwt_write32bitoffsetreg(
      DX_TIME_ID, 0, starttime);  // Note: bit 0 of this register is ignored
...
static void dwt_enable_rf_tx(uint32_t channel, uint8_t switch_control) {
  ...
  if (channel == SEL_CHANNEL5) {
    dwt_or32bitoffsetreg(
        RF_ENABLE_ID, 0,
        (RF_ENABLE_TX_SW_EN_BIT_MASK | RF_ENABLE_TX_CH5_BIT_MASK |
         RF_ENABLE_TX_EN_BIT_MASK | RF_ENABLE_TX_EN_BUF_BIT_MASK |
...
int8_t dwt_do_aes(dwt_aes_job_t *job, dwt_aes_core_type_e core_type) {
  ...
  if ((job->dst_port == AES_Dst_Rx_buf_0) ||
    ...
    if (job->payload_len >
        STS_LEN_128BIT)  //when writing to STS registers (destination port) payload cannot exceed 16 bytes
    {
