...
#define dwt_or8bitoffsetreg(addr, offset, or_val) \
  dwt_modify8bitoffsetreg(addr, offset, -1, or_val)
#define dwt_and8bitoffsetreg(addr, offset, and_val) \
  dwt_modify8bitoffsetreg(addr, offset, and_val, 0)
#define dwt_and_or8bitoffsetreg(addr, offset, and_val, or_val) \
  dwt_modify8bitoffsetreg(addr, offset, and_val, or_val)
#define dwt_set_bit_num_8bit_reg(addr, bit_num) \
  dwt_modify8bitoffsetreg(addr, 0, -1, DWT_BIT_MASK(bit_num))
#define dwt_clr_bit_num_8bit_reg(addr, bit_num) \
  dwt_modify8bitoffsetreg(addr, 0, ~DWT_BIT_MASK(bit_num), 0)

#define dwt_or16bitoffsetreg(addr, offset, or_val) \
  dwt_modify16bitoffsetreg(addr, offset, -1, or_val)
#define dwt_and16bitoffsetreg(addr, offset, and_val) \
  dwt_modify16bitoffsetreg(addr, offset, and_val, 0)
#define dwt_and_or16bitoffsetreg(addr, offset, and_val, or_val) \
  dwt_modify16bitoffsetreg(addr, offset, and_val, or_val)
#define dwt_set_bit_num_16bit_reg(addr, bit_num) \
  dwt_modify16bitoffsetreg(addr, 0, -1, DWT_BIT_MASK(bit_num))
#define dwt_clr_bit_num_16bit_reg(addr, bit_num) \
  dwt_modify16bitoffsetreg(addr, 0, ~DWT_BIT_MASK(bit_num), 0)

#define dwt_or32bitoffsetreg(addr, offset, or_val) \
  dwt_modify32bitoffsetreg(addr, offset, -1, or_val)
#define dwt_and32bitoffsetreg(addr, offset, and_val) \
  dwt_modify32bitoffsetreg(addr, offset, and_val, 0)
#define dwt_and_or32bitoffsetreg(addr, offset, and_val, or_val) \
  dwt_modify32bitoffsetreg(addr, offset, and_val, or_val)
#define dwt_set_bit_num_32bit_reg(addr, bit_num) \
  dwt_modify32bitoffsetreg(addr, 0, -1, DWT_BIT_MASK(bit_num))
#define dwt_clr_bit_num_32bit_reg(addr, bit_num) \
  dwt_modify32bitoffsetreg(addr, 0, ~DWT_BIT_MASK(bit_num), 0)
...
#define DWT_A0_DEV_ID \
  (0xDECA0300)  //!< DW3000 MPW A0 (non PDOA) silicon device ID
#define DWT_A0_PDOA_DEV_ID \
  (0xDECA0310)  //!< DW3000 MPW A0 (with PDOA) silicon device ID
#define DWT_B0_DEV_ID \
  (0xDECA0301)  //!< DW3000 MPW B0 (non PDOA) silicon device ID
#define DWT_B0_PDOA_DEV_ID \
  (0xDECA0311)  //!< DW3000 MPW B0 (with PDOA) silicon device ID
#define DWT_C0_DEV_ID \
  (0xDECA0302)  //!< DW3000 MPW C0 (non PDOA) silicon device ID
#define DWT_C0_PDOA_DEV_ID \
  (0xDECA0312)  //!< DW3000 MPW C0 (with PDOA) silicon device ID

#define DELAY_20uUSec (20) /*Delay of 20uSec(measured 24uSec)*/
#define MAX_RETRIES_FOR_PLL (6)
#define MAX_RETRIES_FOR_PGF (3)

typedef enum { AOA, NON_AOA } dw3000type_e;
...
#define CMD_CCA_TX_W4R \
  0x11  //!< Check if channel clear prior to TX, enable RX when TX done
#define CMD_DTX_REF_W4R \
  0x10  //!< Start delayed TX (as DTX_REF below), enable RX when TX done
#define CMD_DTX_RS_W4R \
  0xF  //!< Start delayed TX (as DTX_RS below), enable RX when TX done
#define CMD_DTX_TS_W4R \
  0xE  //!< Start delayed TX (as DTX_TS below), enable RX when TX done
#define CMD_DTX_W4R \
  0xD  //!< Start delayed TX (as DTX below), enable RX when TX done
...
#define CMD_DTX_REF \
  0x9  //!< Start delayed TX (RMARKER will be @ time = DREF_TIME + DX_TIME)
#define CMD_DRX_RS 0x8  //!< Enable RX @ time = RX_TIME + DX_TIME
#define CMD_DTX_RS \
  0x7  //!< Start delayed TX (RMARKER will be @ time = RX_TIME + DX_TIME)
#define CMD_DRX_TS 0x6  //!< Enable RX @ time = TX_TIME + DX_TIME
#define CMD_DTX_TS \
  0x5  //!< Start delayed TX (RMARKER will be @ time = TX_TIME + DX_TIME)
#define CMD_DRX 0x4  //!< Enable RX @ time specified in DX_TIME register
#define CMD_DTX \
  0x3  //!< Start delayed TX (RMARKER will be @ time set in DX_TIME register)
#define CMD_RX 0x2  //!< Enable RX
#define CMD_TX 0x1  //!< Start TX
#define CMD_TXRXOFF \
  0x0  //!< Turn off TX or RX, clear any TX/RX events and put DW3000 into IDLE
...
#define DWT_SFD_COUNT_WARN \
  (0x2000 >> 7)  // SFD count warning bit (STS quality status bit)
...
#define DWT_START_TX_DELAYED \
  0x01  //! Send the frame at specified time (time must be less that half period away)
#define DWT_RESPONSE_EXPECTED \
  0x02  //! Will enable the receiver after TX has completed
#define DWT_START_TX_DLY_REF \
  0x04  //! Send the frame at specified time (time in DREF_TIME register + any time in DX_TIME register)
#define DWT_START_TX_DLY_RS \
  0x08  //! Send the frame at specified time (time in RX_TIME_0 register + any time in DX_TIME register)
#define DWT_START_TX_DLY_TS \
  0x10  //! Send the frame at specified time (time in TX_TIME_LO register + any time in DX_TIME register)

#define DWT_START_TX_CCA \
  0x20  //! Send the frame if no preamble detected within PTO time
...
#define DWT_START_RX_DELAYED \
  0x01  //! Set up delayed RX, if "late" error triggers, then the RX will be enabled immediately
#define DWT_IDLE_ON_DLY_ERR \
  0x02  //! If delayed RX failed due to "late" error then if this \
      //! flag is set the RX will not be re-enabled immediately, and device will be in IDLE when function exits
#define DWT_START_RX_DLY_REF \
  0x04  //! Enable the receiver at specified time (time in DREF_TIME register + any time in DX_TIME register)
#define DWT_START_RX_DLY_RS \
  0x08  //! Enable the receiver at specified time (time in RX_TIME_0 register + any time in DX_TIME register)
#define DWT_START_RX_DLY_TS \
  0x10  //! Enable the receiver at specified time (time in TX_TIME_LO register + any time in DX_TIME register)
...
#define GPIO_PIN2_RXLED \
  (((uint32_t)0x1) << 6) /* The pin operates as the RXLED output */
#define GPIO_PIN3_TXLED \
  (((uint32_t)0x1) << 9) /* The pin operates as the TXLED output */

#define GPIO_PIN0_EXTTXE \
  (((uint32_t)0x2)       \
   << 0) /* The pin operates as the EXTTXE output (output TX state) */
#define GPIO_PIN1_EXTRXE \
  (((uint32_t)0x2)       \
   << 3) /* The pin operates as the EXTRXE output (output RX state) */

#define GPIO_PIN4_EXTDA \
  (((uint32_t)0x1) << 12) /* The pin operates to support external DA/PA */
#define GPIO_PIN5_EXTTX \
  (((uint32_t)0x1) << 15) /* The pin operates to support external PA */
#define GPIO_PIN6_EXTRX \
  (((uint32_t)0x1) << 18) /* The pin operates to support external LNA */
...
#define DWT_FF_COORD_EN \
  0x100  // behave as coordinator (can receive frames with no dest address (PAN ID has to match))
#define DWT_FF_IMPBRCAST_EN 0x200  // allow MAC implicit broadcast
#define DWT_FF_MAC_LE0_EN \
  0x408  // mac frames allowed if address in LE0_PEND mathces source address
#define DWT_FF_MAC_LE1_EN \
  0x808  // mac frames allowed if address in LE1_PEND mathces source address
#define DWT_FF_MAC_LE2_EN \
  0x1008  // mac frames allowed if address in LE2_PEND mathces source address
#define DWT_FF_MAC_LE3_EN \
  0x2008  // mac frames allowed if address in LE3_PEND mathces source address

// SYS_STATE_LO register errors
#define DW_SYS_STATE_TXERR \
  0xD0000  // TSE is in TX but TX is in IDLE in SYS_STATE_LO register
...
#define DWT_INT_RX                                                            \
  (DWT_INT_LDED | DWT_INT_RFCG | DWT_INT_RPHE | DWT_INT_RFCE | DWT_INT_RFSL | \
   DWT_INT_RFTO | DWT_INT_LDEERR | DWT_INT_RXPTO | DWT_INT_SFDT |             \
   DWT_INT_ARFE)
...
#define DWT_CONFIG \
  0x0001  // download the AON array into the HIF (configuration download)

#define DWT_PRES_SLEEP \
  0x20  // allows for SLEEP_EN bit to be "preserved", although it will self - clear on wake up
...
#define DWT_SLEEP \
  0x2  // enable sleep (if this bit is clear the device will enter deep sleep)
...
#define AON_SLPCNT_LO \
  (0x102)  //address of SLEEP counter bits [19:12] in AON memory
#define AON_SLPCNT_HI \
  (0x103)  //address of SLEEP counter bits [27:20] in AON memory
...
#define AON_SLPCNT_CAL_HI \
  (0x10F)  //address of SLEEP counter cal value high byte

#define DW_CIA_DIAG_LOG_MAX \
  (0x8)  //CIA to copy to swinging set a maximum set of diagnostic registers in Double Buffer mode
#define DW_CIA_DIAG_LOG_MID \
  (0x4)  //CIA to copy to swinging set a medium set of diagnostic registers in Double Buffer mode
#define DW_CIA_DIAG_LOG_MIN \
  (0x2)  //CIA to copy to swinging set a minimal set of diagnostic registers in Double Buffer mode
#define DW_CIA_DIAG_LOG_ALL (0x1)  //CIA to log all diagnostic registers
#define DW_CIA_DIAG_LOG_OFF \
  (0x0)  //CIA to log reduced set of diagnostic registers
...
typedef struct {
  ...
  uint16_t
      status_hi;  //initial value of register as ISR is entered, if relevant for that event type
...
#define GET_STS_REG_SET_VALUE(x) \
  ((uint16_t)1                   \
   << ((x) +                     \
       2)) /* Returns the value to set in CP_CFG0_ID for STS length. The x is the enum value from dwt_sts_lengths_e */

/* Enum used for selecting channel for DGC on-wake kick. */
typedef enum { DWT_DGC_SEL_CH5 = 0, DWT_DGC_SEL_CH9 } dwt_dgc_chan_sel;
...
typedef struct {
  ...
  uint8_t
      sfdType;  //!< SFD type (0 for short IEEE 8-bit standard, 1 for DW 8-bit, 2 for DW 16-bit, 3 for 4z BPRF)
  uint8_t dataRate;  //!< Data rate {DWT_BR_850K or DWT_BR_6M8}
  uint8_t
      phrMode;  //!< PHR mode {0x0 - standard DWT_PHRMODE_STD, 0x3 - extended frames DWT_PHRMODE_EXT}
  uint8_t
      phrRate;  //!< PHR rate {0x0 - standard DWT_PHRRATE_STD, 0x1 - at datarate DWT_PHRRATE_DTA}
  uint16_t sfdTO;   //!< SFD timeout value (in symbols)
  uint8_t stsMode;  //!< STS mode (no STS, STS before PHR or STS after data)
  dwt_sts_lengths_e
      stsLength;  //!< STS length (the allowed values are listed in dwt_sts_lengths_e
...
typedef struct {
  ...
  uint32_t
      ipatovPeak;  // index and amplitude of peak sample in Ipatov sequence CIR
  uint32_t
  ...
  uint16_t
      sts2Power;  // channel area allows estimation of channel power for the STS
...
typedef struct {
  ...
  uint8_t
      OVER;  //8-bit number of receive buffer overrun events (used in double buffer mode)
  ...
  uint8_t
      HPW;  //8-bit half period warning events (when delayed RX/TX enable is used)
...
typedef enum { AES_KEY_No_Load = 0, AES_KEY_Load } dwt_aes_key_load_e;
...
/* Operation selection */
typedef enum { AES_Encrypt = 0, AES_Decrypt } dwt_aes_mode_e;
...
typedef struct {
  dwt_aes_key_otp_type_e
      aes_key_otp_type;  //!< Using KEY from OTP or RAM, if this is set to AES_key_OTP, KEY from OTP is used
  dwt_aes_core_type_e aes_core_type;  //!< Core type GCM or CCM*
  dwt_mic_size_e mic;                 //!< Message integrity code size
  dwt_aes_key_src_e
      key_src;  //!< Location of the key: either as programmed in registers(128 bit) or in the RAM or in the OTP
  dwt_aes_key_load_e
      key_load;      //!< Loads key from RAM or uses KEY from the registers
  uint8_t key_addr;  //!< Address offset of AES key when using AES key in RAM
  dwt_aes_key_size_e
      key_size;  //!< AES key length configuration corresponding to AES_KEY_128/192/256bit
...
static void dwt_xfer3000(
    uint32_t
        regFileID,  //0x0, 0x04-0x7F ; 0x10000, 0x10004, 0x10008-0x1007F; 0x20000 etc
    uint16_t indx,  //sub-index, calculated from regFileID 0..0x7F,
    uint16_t length, uint8_t *buffer, spi_modes_e mode);
 ...
 */
uint8_t dwt_generatecrc8(const uint8_t *byteArray, uint8_t flen,
                         uint8_t crcInit);
...
void dwt_writetodevice(
    uint32_t
        recordNumber,  // input parameter - ID of register file or buffer being accessed
    uint16_t
...
void dwt_readfromdevice(
    ...
    uint8_t *
        buffer  // input parameter - pointer to buffer in which to return the read data.
);
...
void dwt_modify32bitoffsetreg(const uint32_t regFileID,
                              const uint16_t regOffset, const uint32_t _and,
                              const uint32_t _or);
...
void dwt_modify16bitoffsetreg(const uint32_t regFileID,
                              const uint16_t regOffset, const uint16_t _and,
                              const uint16_t _or);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
extern int writetospiwithcrc(uint16_t headerLength, const uint8_t *headerBuffer,
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
extern int writetospi(uint16_t headerLength, uint8_t *headerBuffer,
 ...
 */
extern int readfromspi(uint16_t headerLength, uint8_t *headerBuffer,
