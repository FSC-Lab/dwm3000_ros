...
uint32_t mac_frame_get_aux_frame_cnt(
  ...
  for (cnt = 0; cnt < AUX_FRAME_CNT_SIZE; cnt++) {
    frame_cnt |= (uint32_t)MAC_FRAME_AUX_FRAME_CNT_802_15_4(mac_frame_ptr, cnt)
                 << (cnt * 8);
  }
  return frame_cnt;
}
...
aes_results_e rx_aes_802_15_4(mac_frame_802_15_4_format_t *mac_frame_ptr,
  ...
  if ((frame_length - FCS_LEN) >= aes_job->header_len) {
    ...
    dwt_readrxdata((uint8_t *)MHR_802_15_4_PTR(mac_frame_ptr),
                   aes_job->header_len, 0);
    ...
    if (aes_job->mic_size == MIC_ERROR) return AES_RES_ERROR_FRAME;

    payload_len =
        frame_length -
        (aes_job->header_len + aes_job->mic_size +
         FCS_LEN); /* to get unencrypted payload length subtract MIC, FCS and MHR lengths */
    ...
    dwt_set_keyreg_128(
        &aes_key_ptr[MAC_FRAME_AUX_KEY_IDENTIFY_802_15_4(mac_frame_ptr) - 1]);
