...
void reset() {
  if (_rst == 0xff) {
    ...
    delay(
        2);  // dw1000 data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
    pinMode(_rst, INPUT);
    delay(
        10);  // dwm1000 data sheet v1.2 page 5: nominal 3 ms, to be safe take more time
...
boolean getBit(byte data[], uint16_t n, uint16_t bit) {
  ...
  return bitRead(targetByte,
                 shift);  // TODO wrong type returned byte instead of boolean
}
...
void writeValueToBytes(byte data[], int32_t val, uint16_t n) {
  ...
  for (i = 0; i < n; i++) {
    data[i] =
        ((val >> (i * 8)) & 0xFF);  // TODO bad types - signed unsigned problem
  }
}
...
void clearInterrupts() { memset(_sysmask, 0, LEN_SYS_MASK); }
...
[[deprecated]] uint32_t port_GetEXT_IRQStatus(void) {
  return UINT32_MAX;  // Silence the error for now
}

[[deprecated]] uint32_t port_CheckEXT_IRQ(void) {
  return UINT32_MAX;  // Silence the error for now
}
