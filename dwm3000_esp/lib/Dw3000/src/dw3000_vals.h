...
#define FCS_LEN (2)
#define RX_BUFFER_0_ID \
  0x120000 /* Default Receive Data Buffer (and the 1st of the double buffer set) */
#define RX_BUFFER_1_ID \
  0x130000 /* 2nd Receive Data Buffer (when operating in double buffer mode) */
...
#define DWT_AUTO_CLKS \
  (0x200 | 0x200000 | \
   0x100000)  //this is the default value of CLK_CTRL register
...
#define INDIRECT_POINTER_A_ID \
  0x1D0000 /* pointer to access indirect access buffer A */
#define INDIRECT_POINTER_B_ID \
  0x1E0000 /* pointer to access indirect access buffer B */
...
#define CIA_TDOA_LEN \
  6 /* TDOA value is 41 bits long. You will need to read 6 bytes and mask with 0x01FFFFFFFFFF */
...
#define DB_MAX_DIAG_SIZE \
  232 /* size of diagnostic data (in bytes) when DW_CIA_DIAG_LOG_MAX is set */
#define DB_MID_DIAG_SIZE \
  56 /* size of diagnostic data (in bytes) when DW_CIA_DIAG_LOG_MID is set */
#define DB_MIN_DIAG_SIZE \
  32 /* size of diagnostic data (in bytes) when DW_CIA_DIAG_LOG_MIN is set */
...
#define DWT_TX_BUFF_OFFSET_ADJUST \
  128  // TX buffer offset adjustment when txBufferOffset > 127
...
#define RX_FINFO_STD_RXFLEN_MASK \
...
#define RDB_STATUS_CLEAR_BUFF0_EVENTS                           \
  (RDB_STATUS_CP_ERR0_BIT_MASK | RDB_STATUS_CIADONE0_BIT_MASK | \
   RDB_STATUS_RXFR0_BIT_MASK | RDB_STATUS_RXFCG0_BIT_MASK)
/* RX events mask relating to reception into RX buffer 1,  when double buffer is used */
#define RDB_STATUS_CLEAR_BUFF1_EVENTS                           \
  (RDB_STATUS_CP_ERR1_BIT_MASK | RDB_STATUS_CIADONE1_BIT_MASK | \
   RDB_STATUS_RXFR1_BIT_MASK | RDB_STATUS_RXFCG1_BIT_MASK)

/* User defined RX timeouts (frame wait timeout and preamble detect timeout) mask. */
#define SYS_STATUS_ALL_RX_TO \
  (SYS_STATUS_RXFTO_BIT_MASK | SYS_STATUS_RXPTO_BIT_MASK)

/* All RX errors mask. */
#define SYS_STATUS_ALL_RX_ERR                              \
  (SYS_STATUS_RXPHE_BIT_MASK | SYS_STATUS_RXFCE_BIT_MASK | \
...
#define SYS_STATUS_ALL_ND_RX_ERR \
  (SYS_STATUS_CIAERR_BIT_MASK | SYS_STATUS_RXSTO_BIT_MASK)

/* All RX events after a correct packet reception mask. */
#define SYS_STATUS_ALL_RX_GOOD                              \
  (SYS_STATUS_RXFR_BIT_MASK | SYS_STATUS_RXFCG_BIT_MASK |   \
   SYS_STATUS_RXPRD_BIT_MASK | SYS_STATUS_RXSFDD_BIT_MASK | \
   SYS_STATUS_RXPHD_BIT_MASK | SYS_STATUS_CIADONE_BIT_MASK)

/* All TX events mask. */
#define SYS_STATUS_ALL_TX                                  \
  (SYS_STATUS_AAT_BIT_MASK | SYS_STATUS_TXFRB_BIT_MASK |   \
   SYS_STATUS_TXPRS_BIT_MASK | SYS_STATUS_TXPHS_BIT_MASK | \
   SYS_STATUS_TXFRS_BIT_MASK)

#define SYS_STATUS_RXOK \
  (SYS_STATUS_RXFCG_BIT_MASK | SYS_STATUS_CIA_DONE_BIT_MASK)

#define AES_ERRORS                                          \
  (AES_STS_AUTH_ERR_BIT_MASK | AES_STS_TRANS_ERR_BIT_MASK | \
   AES_STS_MEM_CONF_BIT_MASK)
