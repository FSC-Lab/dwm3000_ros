...
void initiator() {
  ...
  if (counter == NUM_NODES - 1) { /* receive msgs from all anchors */
    ...
    for (int i = 0; i < counter; i++) {
      uint8_t* ith_rxbuf = rx_buffer[i];
      resp_msg_get_ts(&ith_rxbuf[MSG_T_REPLY_IDX], &t_reply);
      t_round = rx_ts[i] - tx_ts;
      tof =
          ((t_round - t_reply * (1 - clockOffsetRatio)) / 2.0) * DWT_TIME_UNITS;
      distance = tof * SPEED_OF_LIGHT;

      dwm3000_UWBRange msg = dwm3000_UWBRange_init_zero;
      pb_ostream_t stream =
          pb_ostream_from_buffer(serial_payload, sizeof serial_payload);
      msg.stamp = current_debug_millis;
      msg.source_id = ith_rxbuf[MSG_SID_IDX];
      msg.destination_id = UID;
      msg.num_units = NUM_NODES;
      msg.tof = tof;
      msg.range = distance;

      status = pb_encode(&stream, dwm3000_UWBRange_fields, &msg);
      Serial.write(serial_payload, stream.bytes_written);
    }
